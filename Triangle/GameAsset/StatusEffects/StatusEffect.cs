...
namespace SlimeGame.GameAsset.StatusEffects
{
    internal interface StatusEffect
    {
        ...
        protected virtual bool Update(ObjectManager objectManager) => false;
        protected virtual bool EffectEnd(ObjectManager objectManager) => false;
        ...
        public static void UpdateAllEffects(ObjectManager objectManager)
        {
            for (int i = 0; i < _activeEffects.Count; i++)
            {
                ...
                if (effect.TimeUntilExpire < 0)
                {
                    ...
                    effect.EffectEnd(objectManager);
                    i--;
                    continue;
                }
                if (effect.Update(objectManager))
                {
					effect.EffectEnd(objectManager);
                    i--;
                    continue;
